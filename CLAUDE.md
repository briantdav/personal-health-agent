...
5. **`src/web/metrics.py`** — the dashboard view model, and the one place that reads *both* live Garmin data and `src/history/db.py` (everywhere else picks one or the other). Sleep hours, sleep score, recovery score, and HRV are keyed to `day` (Garmin already attributes the overnight sleep/morning readiness reading to the wake-up date); miles and workout time are keyed to `day - 1` — a summary of yesterday's training, not an in-progress live count — and every 7d/30d window (both directions: totals for miles/workout, trailing averages + up/down/flat trend for the other four) is anchored to that same `day - 1`, so nothing mixes a partial "today" into a rolling window. `_running_miles()` only counts Garmin's running family (`garmin.is_running_activity`) — golf, walks, rides etc. have a GPS distance too, but aren't running mileage; `_workout_hours()` deliberately counts every activity type. `get_todays_training_plan()` is a placeholder — where `src/agent.py`'s real recommendation logic should eventually plug in.
